...
function injectRuleProviders(config) {
  ...
  config["rule-providers"]["my-direct"] = { behavior: "classical", format: "yaml", interval: 3600, path: "./ruleset/my-direct.yaml", type: "http", url: "https://raw.githubusercontent.com/meme2046/data/main/clash/direct.yaml?_t={{timestamp}}" };config["rule-providers"]["my-proxy"] = { behavior: "classical", format: "yaml", interval: 3600, path: "./ruleset/my-proxy.yaml", type: "http", url: "https://raw.githubusercontent.com/meme2046/data/main/clash/proxy.yaml?_t={{timestamp}}" };config["rule-providers"]["my-reject"] = { behavior: "classical", format: "yaml", interval: 3600, path: "./ruleset/my-reject.yaml", type: "http", url: "https://raw.githubusercontent.com/meme2046/data/main/clash/reject.yaml?_t={{timestamp}}" };const META = 'https://fastly.jsdelivr.net/gh/MetaCubeX/meta-rules-dat@meta/geo';
...
function injectRules(config) {const myCustomRules = ["RULE-SET,my-direct,DIRECT", "RULE-SET,my-reject,REJECT", "RULE-SET,my-proxy,🌍 全球节点"];
  config.rules = [...myCustomRules,
